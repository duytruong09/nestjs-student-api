...
export default class RolesGuard implements CanActivate {
  ...
  async canActivate(context: ExecutionContext): Promise<boolean> {
    ...
    if (role === RoleUserEnum.manager) {
      const isValidRoleUser = await this.checkRoleCustomer(
        request,
        userId,
        url,
        method,
      ...
      if (isValidRoleUser) return isValidRoleUser;

      const isValidRoleAdmin = await this.checkRoleManager(userId, url, method);
