...
@WebSocketGateway(6021, { cors: true })
...
implements OnGatewayConnection, OnGatewayDisconnect, OnGatewayInit {
  ...
  // @UseGuards(JwtWSAccessGuard)
  @SubscribeMessage('share_screen')
  handleShareScreenSocket(@MessageBody() data: string) {
    /* eslint no-console: 0 */
    console.log('SubscribeMessage event', data);
    this.server?.sockets.emit('share_screen', data);
  }
  ...
  // @UseGuards(JwtWSAccessGuard)
  @SubscribeMessage('fooding_socket')
  handleFoodingSocket(@MessageBody() data: string) {
    /* eslint no-console: 0 */
    console.log('SubscribeMessage event', data);
    this.server?.sockets.emit('fooding_socket', data);
  }
}
