import {
  Body, Controller, Delete, Get, HttpCode, NotFoundException, Param, Post, Put, Query, Request, UseInterceptors,
} from '@nestjs/common';
...
import { GroupDetaiType } from '@authorization/a1-user/types/group-detail.type';
...
export default class GroupDetailController {
  ...
  async create(@Body() body: CreateGroupDetailDto): Promise<any> {
    ...
    const groupDetailItem: GroupDetaiType = {
      idGroupDetail: newGroupDetail._id,
      accessMethods: [MethodRouteEnum.GET, MethodRouteEnum.POST, MethodRouteEnum.PUT, MethodRouteEnum.DELETE],
    };

    await this.userService.updateOneBy({ email: adminConstants.email }, { $addToSet: { groupDetails: groupDetailItem } });
  ...
  @Put(':id/add-childs')
  @HttpCode(200)
  async addChilds(@Param('id', ParseObjectIdPipe) id: Types.ObjectId, @Body() body: UpdateChildsDto): Promise<any> {
    const { childs, ...rest } = body;

    const updateChildsPromise = this.groupDetailService.updateManyBy({ _id: { $in: childs } }, { isChild: true });
    const updateGroupDetailPromise = this.groupDetailService.updateOneById(id, {
      ...rest,
      $addToSet: { childs },
    });

    const [_, updateGroupDetail] = await Promise.all([updateChildsPromise, updateGroupDetailPromise]);
  ...
  @HttpCode(200)
  async removeChilds(@Param('id', ParseObjectIdPipe) id: Types.ObjectId, @Body() body: UpdateChildsDto): Promise<any> {
    const { childs, ...rest } = body;

    const updateChildsPromise = this.groupDetailService.updateManyBy({ _id: { $in: childs } }, { isChild: false });
    const updateGroupDetailPromise = this.groupDetailService.updateOneById(id, {
      ...rest,
  ...
  @Put(':id')
  @HttpCode(200)
  async update(@Param('id', ParseObjectIdPipe) id: Types.ObjectId, @Body() body: UpdateGroupDetailDto): Promise<any> {
  ...
  async deleteManyByIds(@Param('ids') ids: string): Promise<any> {
    ...
    for (let index = 0; index < listIds.length; index += 1) {
      ...
      deleteAndPullInRelatedCollectionPromises.push(this.deleteAndPullInRelatedCollection(new Types.ObjectId(id)));
    }
   ...
   */
  @Get('collection-key')
  async findCollectionKeyByCollectionName(@Query('collectionName') collectionName: string) {
  ...
  async getAllGroupDetailsOfUser(@GetCurrentUserId() id: Types.ObjectId, @ApiQueryParams() query: any) {
    ...
    const groupDetailIdsInUser = groupDetailsInUser.map((groupDetailInUser: any) => groupDetailInUser.idGroupDetail);

    const groupDetailIdsInGroups: any[] = [];
    // if have groups => add groupDetails
    groups.forEach((group: any) => {
      const groupDetailIdsInGroup = group.groupDetails.map((groupDetailInGroup: any) => groupDetailInGroup.idGroupDetail);

      groupDetailIdsInGroups.push(...groupDetailIdsInGroup);
    });

    // Unique groupDetails ids
    const groupDetailsIdsUnique = [...new Set([...groupDetailIdsInUser, ...groupDetailIdsInGroups])];
  ...
  @HttpCode(200)
  async getAccessMethodsOfUserLoggedInByCollectionName(@GetCurrentUserId() id: Types.ObjectId, @Query('link') link: string): Promise<any> {
    ...
    const groupDetailsExist = groupDetails.find((item) => item.idGroupDetail.toString() === groupDetail._id.toString());
    ...
    if (groups.length > 0) {
      const groupExist = await this.groupService.findOneBy({
        $and: [{ _id: { $in: groups } }, { 'groupDetails.idGroupDetail': groupDetail._id }],
      });

      if (groupExist) {
        const groupDetailInGroup = groupExist.groupDetails.find((item: any) => item.idGroupDetail.toString() === groupDetail._id.toString());

        // update access methods
        accessMethods = sortAndUniqueMethods([...accessMethods, ...sortAndUniqueMethods(groupDetailInGroup.accessMethods)]);
  ...
  async deleteAndPullInRelatedCollection(id: Types.ObjectId) {
    ...
    if (groupDetail.childs.length >= 0) {
      pullGroupDetailsPromise.push(this.groupDetailService.moveChildToMain(groupDetail.childs));
    }
