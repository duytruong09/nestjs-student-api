...
export default class GroupApiController {
  ...
  @Put(':id')
  @HttpCode(200)
  async update(@Param('id', ParseObjectIdPipe) id: Types.ObjectId, @Body() body: UpdateGroupApiDto): Promise<any> {
  ...
  async deleteManyByIds(@Param('ids') ids: string): Promise<any> {
    ...
    for (let index = 0; index < listIds.length; index += 1) {
      ...
      pullPromises.push(pullGroupAPIAccessesInGroup, pullGroupAPIDeniesInGroup, pullGroupAPIAccessesInUser, pullGroupAPIDeniesInUser);
    }
    const _ids = ids.split(',').map((item: any) => new Types.ObjectId(item));
    const groupApisDeletedPromise = this.groupApiService.deleteManyHardByIds(_ids);
