import {
  Controller, UseInterceptors, Body, Delete, Get, HttpCode, NotFoundException, Param, Post, Put, Query, Req,
} from '@nestjs/common';
...
export default class AuthUserAccessController {
  ...
  @Put(':id')
  @HttpCode(200)
  async update(@Param('id', ParseObjectIdPipe) id: Types.ObjectId, @Body() body: UpdateAuthUserAccessDto): Promise<any> {
  ...
  async deleteManyByIds(@Param('ids') ids: string): Promise<any> {
    ...
    const result = await this.authUserAccessService.deleteManyHardByIds(_ids);
    return result;
  }
