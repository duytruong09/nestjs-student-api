...
export default class UploadController {
  constructor(private uploadLocalService: UploadLocalService, private uploadS3Service: UploadS3Service, private storageService: StorageService) {}
  ...
  async uploadMultipleFilesLocal(@UploadedFiles() files: Express.Multer.File[]) {
    const result = [];
    for (let i = 0; i < files.length; i += 1) {
      result.push(files[i].path.replace('public/', `${ShareFunction.getClientUrl()}/static/`));
    }
    return { files: result };
  }
  ...
  async uploadMultipleVideosLocal(@UploadedFiles() videos: Express.Multer.File[]) {
    const result = [];
    for (let i = 0; i < videos.length; i += 1) {
      result.push(videos[i].path.replace('public/', `${ShareFunction.getClientUrl()}/static/`));
    }
    return { files: result };
  }
  ...
  async confirmFilesWasUsedLocal(
    ...
    files.forEach((file) => {
      ...
      if (file.startsWith(`${ShareFunction.getClientUrl()}/static/upload/image/`)) {
    ...
    const filesWasUsedPromise = fileToConfirm.map((file) => {
      ...
      return this.uploadLocalService.confirmFileWasUsed(file);
    });
   ...
   */
  @Post('s3-tmp/')
  async confirmFilesWasUsedS3(@GetCurrentUserId() userId: string, @Body('files') files: string[]) {
    // save list file
    const filesWasUsedPromise = files.map((file) => {
      return this.uploadS3Service.confirmFileWasUsed(new Types.ObjectId(userId), file);
    });
